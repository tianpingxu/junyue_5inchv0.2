...
/** @defgroup ETH_Exported_Functions ETH导出函数
 *  @ingroup ETHERNET
 * @{
 */

/** @defgroup ETH_Exported_Functions_Group1 初始化和取消初始化函数
  *  @brief   初始化和配置函数
  *
  @verbatim
  ===============================================================================
...
/**
 * @brief  根据默认值初始化以太网MAC和DMA
 * @param  指向包含以下内容的ETH_HandleTypeDef结构的指针
 * @retval HAL status
 */
status_t HAL_ETH_Init(ETH_HandleTypeDef *heth)
{
    ...
    if ((HAL_ETH_WritePHYRegister(heth, PHY_BCR, PHY_RESET)) != STATUS_OK)
    {
    ...
    if ((heth->Init).AutoNegotiation != ETH_AUTONEGOTIATION_DISABLE)
    {
        ...
        if ((HAL_ETH_WritePHYRegister(heth, PHY_BCR, PHY_AUTONEGOTIATION)) != STATUS_OK)
        {
            /* In case of write timeout */
            err = ETH_ERROR;
        ...
        if ((HAL_ETH_ReadPHYRegister(heth, PHY_SR, &phyreg)) != STATUS_OK)
        {
            /* In case of write timeout */
            err = ETH_ERROR;
    ...
    else /* AutoNegotiation Disable */
    {
        ...
        if (HAL_ETH_WritePHYRegister(heth, PHY_BCR, ((uint16_t)((heth->Init).DuplexMode >> 3U) | (uint16_t)((heth->Init).Speed >> 1U))) != STATUS_OK)
        {
            /* In case of write timeout */
            err = ETH_ERROR;
    ...
    // jshi add 20200924
    __HAL_ETH_MMC_RX_IT_DISABLE(heth, 0xFFFFFFFFU);
    __HAL_ETH_MMC_TX_IT_DISABLE(heth, 0xFFFFFFFFU);
    __HAL_ETH_MMC_IPC_IT_DISABLE(heth, 0xFFFFFFFFU);
...
/**
 * @brief  以chain mode初始化DMA Tx描述符
 * @param  指向ETH_HandleTypeDef结构的指针，该结构包含ETHERNET模块的配置信息
 * @param  DMATxDescTab指向第一个Tx desc列表的指针
 * @param  TxBuff指向第一个TxBuffer列表的指针
 * @param  TxBuffCount列表中使用的Tx desc的编号
 * @retval HAL status
 */
status_t HAL_ETH_DMATxDescListInit(ETH_HandleTypeDef *heth, ETH_DMADescTypeDef *DMATxDescTab, uint8_t *TxBuff, uint32_t TxBuffCount)
{
    ...
    for (i = 0U; i < TxBuffCount; i++)
    {
        ...
        printf("%s: size=%x addr=%x next=%x\n", __FUNCTION__, dmatxdesc->ControlBufferSize, dmatxdesc->Buffer1Addr, dmatxdesc->Buffer2NextDescAddr);
    }
...
/**
 * @brief  以chain mode初始化DMA Rx描述符。
 * @param  指向包含以下内容的ETH_HandleTypeDef结构的指针ETHERNET模块的配置信息
 * @param  DMARxDescTab指向第一个Rx desc列表的指针
 * @param  RxBuff指向第一个RxBuffer列表的指针
 * @param  RxBuffCount列表中使用的Rx desc的编号
 * @retval HAL status
 */
status_t HAL_ETH_DMARxDescListInit(ETH_HandleTypeDef *heth, ETH_DMADescTypeDef *DMARxDescTab, uint8_t *RxBuff, uint32_t RxBuffCount)
{
    ...
    if ((heth->Init).EthDMAMode == ETH_DMA_CONTINUOUS_MODE)
    {
        ...
        for (i = 0U; i < RxBuffCount; i++)
        {
            ...
            DMARxDesc->Buffer1Addr = ETH_DMARXDESC_RCH | (ETH_RX_BUF_SIZE);

            /* Set Buffer1 address pointer */
            DMARxDesc->ControlBufferSize = (uint32_t)(&RxBuff[i * (ETH_RX_BUF_SIZE >> 2)]);

            if ((heth->Init).RxMode == ETH_RXINTERRUPT_MODE)
            {
                /* Enable Ethernet DMA Rx Descriptor interrupt */
                DMARxDesc->Buffer1Addr &= ~ETH_DMARXDESC_DIC;
            }
            ...
            printf("%s: size=%x addr=%x next=%x\n", __FUNCTION__, DMARxDesc->ControlBufferSize, DMARxDesc->Buffer1Addr, DMARxDesc->Buffer2NextDescAddr);
        }
    }
    else
    {
    ...
    if ((heth->Init).EthDMAMode == ETH_DMA_CONTINUOUS_MODE)
    {
        (heth->Instance)->DMACRBR = (uint32_t)RxBuff;
        (heth->Instance)->DMACREAR = (uint32_t)(&RxBuff[RxBuffCount * (ETH_RX_BUF_SIZE >> 2)]);
        (heth->Instance)->DMACRHSR = (uint32_t)(RxBuffCount * (ETH_RX_BUF_SIZE >> 2) - 0x600);
        (heth->Instance)->DMACRMR = (0x00000001);
    }
...
/**
 * @brief  初始化ETH MSP
 * @param  param heth指向包含以下内容的ETH_HandleTypeDef结构的指针ETHERNET模块的配置信息
 * @retval None
 */
...
#if (USE_HAL_ETH_REGISTER_CALLBACKS == 1)
/**
  * @brief 注册用户ETH回调,用于代替弱的预定义回调
  * @param heth eth句柄
  * @param CallbackID要注册的回调的ID
  *         此参数可以是下列值之一：
  ...
  * @param pCallback指向回调函数的指针
  * @retval status
  */
status_t HAL_ETH_RegisterCallback(ETH_HandleTypeDef *heth, HAL_ETH_CallbackIDTypeDef CallbackID, pETH_CallbackTypeDef pCallback)
{
...
/**
 * @brief 取消注册ETH回调
 *        ETH callabck被重定向到弱预定义的回调
 * @param heth eth句柄
 * @param CallbackID要取消注册的回调的ID
 *        此参数可以是下列值之一：
 *          @arg @ref HAL_ETH_TX_COMPLETE_CB_ID Tx Complete Callback ID
 *          @arg @ref HAL_ETH_RX_COMPLETE_CB_ID Rx Complete Callback ID
 *          @arg @ref HAL_ETH_DMA_ERROR_CB_ID      DMA Error Callback ID
 ...
 * @retval status
 */
status_t HAL_ETH_UnRegisterCallback(ETH_HandleTypeDef *heth, HAL_ETH_CallbackIDTypeDef CallbackID)
{
    status_t status = STATUS_OK;
...
/** @defgroup ETH_Exported_Functions_Group2 操作函数
  *  @brief   数据传输函数
  *
  @verbatim
  ==============================================================================
...
status_t HAL_ETH_TransmitFrame(ETH_HandleTypeDef *heth, uint32_t FrameLength)
{
    ...
    if (bufcount == 1U)
    {
        ...
        /* Set frame size */
        heth->TxDesc->ControlBufferSize &= ~(ETH_DMATXDESC_TBS1);
    ...
    if ((heth->Init).EthDMAMode == ETH_DMA_CONTINUOUS_MODE)
    {
        ...
        if (((heth->RxDesc->Status & ETH_DMARXDESC_OWN) == (uint32_t)RESET))
        {
            ...
            if (((heth->RxDesc->Status & ETH_DMARXDESC_LS) != (uint32_t)RESET))
            {
                ...
                heth->RxFrameInfos.buffer = ((heth->RxFrameInfos).FSRxDesc)->ControlBufferSize; // Buffer1Addr
                /* point to next descriptor */
 ...
 * @retval HAL status
 */
extern void ETH_Print(void);
extern void HAL_ETH_RBUCallback(ETH_HandleTypeDef *heth);
extern void HAL_ETH_AISCallback(ETH_HandleTypeDef *heth);
extern void HAL_ETH_BufferBFullCallback(ETH_HandleTypeDef *heth);
extern void HAL_ETH_BufferAFullCallback(ETH_HandleTypeDef *heth);
void HAL_ETH_IRQHandler(ETH_HandleTypeDef *heth)
{

    if (__HAL_ETH_DMA_GET_FLAG(heth, ETH_DMA_FLAG_BBF))
    {
        // printf("HAL_ETH_IRQHandler ETH_DMA_FLAG_BBF \n");
        __HAL_ETH_DMA_CLEAR_IT(heth, ETH_DMA_FLAG_BBF);
        // HAL_ETH_BufferBFullCallback(heth);
    }

    if (__HAL_ETH_DMA_GET_FLAG(heth, ETH_DMA_FLAG_BAF))
    {
        // printf("HAL_ETH_IRQHandler ETH_DMA_FLAG_BAF \n");
        __HAL_ETH_DMA_CLEAR_IT(heth, ETH_DMA_IT_BAF);
        // HAL_ETH_BufferAFullCallback(heth);
    }

    if (__HAL_ETH_DMA_GET_FLAG(heth, ETH_DMA_FLAG_RBU))
    {
        // printf("HAL_ETH_IRQHandler ETH_DMA_FLAG_RBU \n");
        __HAL_ETH_DMA_CLEAR_IT(heth, ETH_DMA_IT_RBU);
        HAL_ETH_RBUCallback(heth);
    }
    ...
    if (__HAL_ETH_DMA_GET_FLAG(heth, ETH_DMA_FLAG_AIS))
    {
        ...
        // printf(" (heth->Instance)->DMASR =0x%08x,(heth->Instance)->DMAIER =0x%08x   \r\n", (heth->Instance)->DMASR,(heth->Instance)->DMAIER);
 ...
 * @brief  读取PHY寄存器
 * @param  指向ETH_HandleTypeDef结构的指针，该结构包含ETHERNET模块的配置信息
 * @param  PHY寄存器地址，是32个PHY寄存器的索引。
 *                此参数可以是下列值之一：
 *                   PHY_BCR: 收发器基本控制寄存器
 *                   PHY_BSR: 收发器基本状态寄存器
 *                   根据所使用的PHY，可以读取更多的PHY寄存器
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth, uint32_t err)
{
    ...
    if ((heth->Init).EthDMAMode == ETH_DMA_CONTINUOUS_MODE)
    {
        ...
        if ((heth->Init).RxMode == ETH_RXINTERRUPT_MODE)
        {
            ...
            //__HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_R| ETH_DMA_IT_BBF | ETH_DMA_IT_BAF );
            //__HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_R | ETH_DMA_IT_BBF | ETH_DMA_IT_BAF | ETH_DMA_IT_RBU | ETH_DMA_IT_AIS );
            //__HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_BBF | ETH_DMA_IT_BAF | ETH_DMA_IT_RBU );
            //__HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_R | ETH_DMA_IT_BBF | ETH_DMA_IT_BAF | ETH_DMA_IT_RBU );
            __HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_R | ETH_DMA_IT_RBU);
        }
    }
    else // ETH_DMA_DEFAULT_MODE
    {
        if ((heth->Init).RxMode == ETH_RXINTERRUPT_MODE)
        {
            /* Enable the Ethernet Rx Interrupt */
 ...
 * @brief  配置选定的MAC地址
 * @param  指向ETH_HandleTypeDef结构的指针，该结构包含ETHERNET模块的配置信息
 * @param  MacAddr要配置的MAC地址
 *          此参数可以是下列值之一:
 *             @arg ETH_MAC_Address0: MAC Address0
 *             @arg ETH_MAC_Address1: MAC Address1
 *             @arg ETH_MAC_Address2: MAC Address2
 *             @arg ETH_MAC_Address3: MAC Address3
 * @param  Addr指向MAC地址缓冲区数据的指针（6个字节）
 * @retval HAL status
 */
static void ETH_MACAddressConfig(ETH_HandleTypeDef *heth, uint32_t MacAddr, uint8_t *Addr)
{
...
/**
 * @brief  此函数根据CPU周期方法提供延迟（以毫秒为单位）
 * @param  mdelay指定延迟时间长度（以毫秒为单位）
 * @retval None
 */
static void ETH_Delay(uint32_t mdelay)
{
    __IO uint32_t Delay = mdelay * (g_core_clk / 8U / 1000U);
    do
    {
        __NOP();
    } while (Delay--);
